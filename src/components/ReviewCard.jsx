...
function ReviewCard() {
  ...
  const handleRatingCallback = (rating) => {
...
// const MovieDetail = ({ movieId }) => {
  ...
  const handleLike = () => {
    ...
    axios.post('http://127.0.0.1:8000/like/', { movie_id: id }, {
      headers: {
        'Authorization': `Token ${localStorage.getItem('token')}`, // Include the user's token
      },
    })
  ...
  const handleUnlike = () => {
    ...
    axios.post('http://127.0.0.1:8000/unlike/', { movie_id: id }, {
      headers: {
        'Authorization': `Token ${localStorage.getItem('token')}`, // Include the user's token
      },
    })
 ...
 const handleRate = (ratingValue) => {
  ...
  .then(() => {console.log("movie successfully rated") })

  .catch((error)=>{console.error('Error rating the movie',error);});
 }; 
 ...
 const handleDeleteRate = () => {
  ...
  axios.post('http://127.0.0.1:8000/delete-rating/', {
    movie_id: id,
    stars: parentRating
  }, {
    headers: {
  ...
  return (
    ...
    <div className="d-flex gap-3 justify-content-between align-items-center">
      <span className="d-flex light-charcoal flex-column gap-1">
        <BsFillEyeFill className='align-self-center' size={50} style={{cursor:"pointer"}}/>
        <h6 className='text-center'>Watch</h6>
      </span>
      <span className="d-flex light-charcoal flex-column gap-1"  onClick={isLiked ? handleUnlike : handleLike}>
        <FaHeart className='align-self-center' size={50} style={{ color: isLiked ? '#F77F00' : '#B6C1DF',cursor:"pointer" }}/>
        <h6 className='text-center'>{isLiked ? 'Liked' : 'Like'}</h6>
      </span>
      <span className="d-flex light-charcoal flex-column gap-1">
        <BsFillStopwatchFill className='align-self-center' size={50} style={{cursor:"pointer"}}/>
    ...
    <h6 className='text-center'data-bs-toggle="modal" data-bs-target="#reviewId" style={{cursor:"pointer",color:"#3A86FF"}}>Review or log...</h6>
   
    
  
    <div class="modal fade" id="reviewId" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false" role="dialog" aria-labelledby="modalTitleId" aria-hidden="true">
      <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered modal-sm modal-md modal-lg modal-xl" role="document">
        <div class="modal-content">
          <div class="modal-header  bg-payne-gray">
            <h5 class="modal-title light-charcoal" id="modalTitleId">make a review</h5>
      ...
      <div ref={quillRef} />
          ...
          <div class="modal-footer  bg-payne-gray">
            ...
            <button type="button" class="btn bg-fire-engine-red " data-bs-dismiss="modal" onClick={handleReviewSubmit}>post review</button>
          </div>
        </div>
      </div>
    </div>
